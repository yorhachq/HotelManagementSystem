...
public class DataStatistics {
     ...
     * @return 对应模块的Map集合
     */
    public static Map<String, Object> getTodayData(String moduleName, Map<String, Object> todayData, List<Map<String, Object>> thisWeekData) {
        String name = "";
        String columnName = "";
        String percent = "";
        // 根据对应模块名称填充相应的数据
        ...
        if (!thisWeekData.isEmpty()) {
            Integer lastDayCount = Convert.toInt(thisWeekData.get(thisWeekData.size() - 2).get(columnName));
            double increase = (value - lastDayCount) * 100.0 / lastDayCount;
            percent = StrUtil.format("{}{}%", increase > 0 ? "+" : "-", String.format("%.2f", increase));
        }
        // 本周数据概览
        String finalColumnName = columnName;
        List<Integer> data = thisWeekData.stream().map(item -> Convert.toInt(item.get(finalColumnName))).toList();
        return Map.of(
                "name", name,
                "value", value,
                "percent", percent,
